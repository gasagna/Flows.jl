...
function _step!{T<:IMEXRK3R2R}(I::Type{T}, g, A, t, Δt, x)
    ...
    if isembedded(I)
        ...
        push!(expr_all.args, broadcast2fields(isQuad, :(@over_i x̂[i] = x[i])))
    end
    ...
    for k = 1:nstages(tab)
        ...
        b̂ᴵk    = tab[Val{:b̂ᴵ}, k]
        if k == 1
            push!(expr.args, broadcast2fields(isQuad, :(@over_i y[i] = x[i])))
        else
            ...
            (aᴵkkm1 - bᴵkm1) == 0 && (aᴱkkm1 - bᴱkm1) == 0 &&
                push!(expr.args, broadcast2fields(isQuad, :(@over_i y[i] = x[i])))
            (aᴵkkm1 - bᴵkm1) == 0 && (aᴱkkm1 - bᴱkm1) != 0 &&
                push!(expr.args, broadcast2fields(isQuad, :(@over_i y[i] = x[i] + $(aᴱkkm1 - bᴱkm1)*Δt*y[i])))
            (aᴵkkm1 - bᴵkm1) != 0 && (aᴱkkm1 - bᴱkm1) == 0 &&
                push!(expr.args, broadcast2fields(isQuad, :(@over_i y[i] = x[i] + $(aᴵkkm1 - bᴵkm1)*Δt*z[i])))
            (aᴵkkm1 - bᴵkm1) != 0 && (aᴱkkm1 - bᴱkm1) != 0 &&
                push!(expr.args, broadcast2fields(isQuad, :(@over_i y[i] = x[i] + $(aᴵkkm1 - bᴵkm1)*Δt*z[i] + $(aᴱkkm1 - bᴱkm1)*Δt*y[i])))
        end
        # compute z = A*y then
        ...
        push!(expr.args, broadcast2fields(isQuad, :(@over_i w[i] = y[i])))
        aᴵkk != 0 && push!(expr.args, broadcast2fields(isQuad, :(@over_i w[i] += $aᴵkk*Δt*z[i])))
        push!(expr.args, :(g(t + $cᴱk*Δt, w, y)))

        bᴵk != 0 && push!(expr.args, broadcast2fields(isQuad, :(@over_i x[i] += $bᴵk*Δt*z[i])))
        bᴱk != 0 && push!(expr.args, broadcast2fields(isQuad, :(@over_i x[i] += $bᴱk*Δt*y[i])))

        if isembedded(I)
            b̂ᴵk != 0 && push!(expr.args, broadcast2fields(isQuad, :(@over_i x̂[i] += $b̂ᴵk*Δt*z[i])))
            b̂ᴱk != 0 && push!(expr.args, broadcast2fields(isQuad, :(@over_i x̂[i] += $b̂ᴱk*Δt*y[i])))
        end
...
function _step!{T<:IMEXRK4R3R}(I::Type{T}, g, A, t, Δt, x)
    ...
    for k = 1:nstages(tab)
        ...
        if k == 1
            push!(expr.args, broadcast2fields(isQuad, :(@over_i  y[i] = x[i])))
            push!(expr.args, broadcast2fields(isQuad, :(@over_i zᴵ[i] = x[i])))
            push!(expr.args, broadcast2fields(isQuad, :(@over_i zᴱ[i] = x[i])))
        else
            aᴱkkm1 = tab[Val{:aᴱ}, k, k-1]
            aᴱkkm1 == 0 && push!(expr.args, broadcast2fields(isQuad, :(@over_i zᴱ[i] = y[i])))
            aᴱkkm1 != 0 && push!(expr.args, broadcast2fields(isQuad, :(@over_i zᴱ[i] = y[i] + $aᴱkkm1*Δt*zᴱ[i])))
            if k < nstages(tab)
                ...
                c1 == 0 && c2 == 0 && 
                    push!(expr.args, broadcast2fields(isQuad, :(@over_i y[i] = x[i])))
                c1 == 0 && c2 != 0 && 
                    push!(expr.args, broadcast2fields(isQuad, :(@over_i y[i] = x[i] + $c2*(zᴱ[i] - y[i]))))
                c1 != 0 && c2 == 0 && 
                    push!(expr.args, broadcast2fields(isQuad, :(@over_i y[i] = x[i] + $c1*Δt*zᴵ[i])))
                c1 != 0 && c2 != 0 && 
                    push!(expr.args, broadcast2fields(isQuad, :(@over_i y[i] = x[i] + $c1*Δt*zᴵ[i] + $c2*(zᴱ[i] - y[i]))))
            end
            aᴵkkm1 = tab[Val{:aᴵ}, k, k-1]
            aᴵkkm1 != 0 && push!(expr.args, broadcast2fields(isQuad, :(@over_i zᴱ[i] = zᴱ[i] + $aᴵkkm1*Δt*zᴵ[i])))
        end
        # compute w = A*zᴱ then
        ...
        push!(expr.args, broadcast2fields(isQuad, :(@over_i w[i] = zᴱ[i])))
        aᴵkk != 0 && push!(expr.args, broadcast2fields(isQuad, :(@over_i w[i] += $aᴵkk*Δt*zᴵ[i])))

        push!(expr.args, :(g(t + $cᴱk*Δt, w, zᴱ)))

        bᴵk != 0 && push!(expr.args, broadcast2fields(isQuad, :(@over_i x[i] += $bᴵk*Δt*zᴵ[i])))
        bᴱk != 0 && push!(expr.args, broadcast2fields(isQuad, :(@over_i x[i] += $bᴱk*Δt*zᴱ[i])))

        if isembedded(I)
            b̂ᴵk != 0 && push!(expr.args, broadcast2fields(isQuad, :(@over_i x̂[i] += $b̂ᴵk*Δt*zᴵ[i])))
            b̂ᴱk != 0 && push!(expr.args, broadcast2fields(isQuad, :(@over_i x̂[i] += $b̂ᴱk*Δt*zᴱ[i])))
        end
        push!(expr_all.args, expr)
    end
    expr_all
